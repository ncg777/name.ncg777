...
public class RhythmMatrix96 {
  ...
  public static void main(String[] args) {
    EventQueue.invokeLater(new Runnable() {
      public void run() {
        try {
          RhythmMatrix96 window = new RhythmMatrix96();
        ...
        new Thread(new Runnable() {
          ...
          public void run() {
            try {
              ...
              for(BeatRhythm r : BeatRhythm.getBeatRhythms()) t0.add(r.getRhythmClicks());
              for(Rhythm r : t0){
                if(pred.apply(r)) {
                  t.add(r);
                }
              ...
              for(int i=0;i<strFixed.length;i++) {
                ...
                BeatRhythmList r = BeatRhythmList.parseBeatRhythmSeq(strFixed[i].trim());
                for(int j=0;j<n;j++) {
                  output.set(i, j, r.get(j%r.size()).getRhythmClicks());
                } 
                fixedSize++;
              }
              m += fixedSize;
              ...
              Matrix<BeatRhythm> tmpMat = new Matrix<BeatRhythm>(m,n);
              for(int i=0;i<m;i++) {
                for(int j=0;j<n;j++) {
                  tmpMat.set(i, j, BeatRhythm.fromRhythmClicks(output.get(i, j)));
                }
              }
