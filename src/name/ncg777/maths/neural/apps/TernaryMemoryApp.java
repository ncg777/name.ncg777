...
public final class TernaryMemoryApp {
  ...
  private final JTextField cue = new JTextField("1?T0", 12);
  ...
  private final JSpinner hidden = new JSpinner(new SpinnerNumberModel(1, 0, 6, 1));
  ...
  private void show() {
    ...
    entry.setBorder(BorderFactory.createTitledBorder("Stored patterns: one per line, 1–6 trits each"));
    ...
    tables.add(tablePanel("Spin model — probability given the cue", spin));
    ...
    train.addActionListener(event -> {
      try {
        ...
        start(() -> TernaryAssociativeMemory.train(rows, count), fitted -> {
          ...
          status.setText("Trained " + rows.length + " examples. Repeated rows count more often. Choose Recall cue or Test 200 damaged cues.");
        });
    ...
    test.addActionListener(event -> {
      try {
        ...
        start(() -> memory.benchmark(200, masked, rate, 777), result -> status.setText(String.format(
