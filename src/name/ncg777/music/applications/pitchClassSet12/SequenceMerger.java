...
import name.ncg777.music.pitchClassSet12.PitchClassSet12List;
  ...
  private void initialize() {
    ...
    btnMerge.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ArrayList<PitchClassSet12List> arr = new ArrayList<>();
        var ss = txtArea.getText().trim().split("\n+");
        for(var s :ss) {arr.add(PitchClassSet12List.parseForte(s));}
        txtResult.setText(PitchClassSet12List.merge(arr).toString((c) -> c.toForteNumberString()));
      }
    });
