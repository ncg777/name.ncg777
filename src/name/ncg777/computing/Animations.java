...
public class Animations {
  ...
  public static Enumeration<BufferedImage> Hadamard20241228_1(int n, int width, int height, int fps, double dur) {
    ...
    return new Enumeration<BufferedImage>() {
      ...
      public BufferedImage nextElement() {
          final double t = (double) k/(double)upper;
          final Function<Double,Double> _f = (Double r) -> -1.0+7.0*(1.0-r)*r;
          var img = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
          var g = img.createGraphics();
          ...
          GraphicsFunctions.drawColorField2D(g, 
              (_x,_y) -> {
                ...
                double v = m.get(
                    (int)(Math.min(Math.abs(x*Math.sqrt(2.0)),1.0)*((double)dim-1)), 
                    (int)(Math.min(Math.abs(y*Math.sqrt(2.0)),1.0)*((double)dim-1))).doubleValue();
                var rfadestart = 0.5;
                var rfadeend = 0.7;
                return new Color(
                  (int)((0.75-0.25*Math.cos(2.0*Math.PI*t))*((1.0-r)*(v*0.5+0.5)*255.0)),
                  (int)((0.75+0.25*Math.cos(2.0*Math.PI*t))*((1.0-r)*(v*0.5+0.5)*255.0)),
                  (int)((0.5*(v*0.5+0.5))*255.0),
                  r > rfadeend ? 0 : (
                      r < rfadestart ? 255 : 
                          ((int)(255.0*
                              (1.0-(
