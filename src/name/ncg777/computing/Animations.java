...
public class Animations {
      ...
      public BufferedImage nextElement() {
        ...
        GraphicsFunctions.drawColorField2D(g, 
            (_x,_y) -> {
              ...
              var rfadeend = 0.7;
              return new Color(
                  (int)(((0.75-0.25*Math.cos(2.0*Math.PI*t))*((v*0.5+0.5)+(0.5-v*0.5))*255.0)*Math.pow(1.0-r,2.0)),
                  (int)(((0.75+0.25*Math.cos(2.0*Math.PI*t))*((v*0.5+0.5)+(0.5-v*0.5))*255.0)*Math.pow(1.0-r,2.0)),
                  (int)(((0.5-v*0.5)*255.0)*Math.pow(1.0-r,2.0)),
                  r > rfadeend ? 0 : (
                      r < rfadestart ? 255 : 
                        ((int)(255.0*
                            (1.0-(
  ...
  public static Enumeration<BufferedImage> MatrixOrb20241229_1(MatrixOfDoubles mat, int width, int height, int fps, double dur) {
    ...
    return new Enumeration<BufferedImage>() {
      ...
      public BufferedImage nextElement() {
        final double t = (double) k/(double)upper;
        final Function<Double,Double> _f = (Double r) -> -1.0+1.0*(1.0-r)*r;
        var img = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        var g = img.createGraphics();
        g.rotate(Math.PI/4.0+Math.PI*2.0*t,width/2,height/2);
        GraphicsFunctions.drawColorField2D(g, 
            (_x,_y) -> {
              ...
              var rfadeend = 0.7;
              return new Color(
                  (int)((((0.75-0.25*Math.cos(2.0*Math.PI*t))*(v*0.5+0.5)*Math.pow(1.0-r,2.0))+(0.5-v*0.5))*255.0),
                  (int)((((0.75+0.25*Math.cos(2.0*Math.PI*t))*(v*0.5+0.5)*Math.pow(1.0-r,2.0))+(0.5-v*0.5))*255.0),
                  (int)((0.5-v*0.5)*255.0),
                  r > rfadeend ? 0 : (
                      r < rfadestart ? 255 : 
